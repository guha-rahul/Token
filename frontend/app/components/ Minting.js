...
const buytoken = async (inputValue) => {
  const provider = new ethers.providers.Web3Provider(window.ethereum, "any");
  // Prompt user for account connections
  await provider.send("eth_requestAccounts", []);
  const signer = provider.getSigner();
  const MyToken = new ethers.Contract(Contract_address, abi, signer);
  value = 0.1 * inputValue;
  let tx = await MyToken.mint(signer.getAddress(), inputValue, {
...
const Minting = () => {
  ...
  return (
    <div className=" w-[38rem] h-[38rem] mx-auto mt-10 rounded-full backdrop-brightness-125 flex justify-center items-center relative">
